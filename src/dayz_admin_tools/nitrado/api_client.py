...
def _validate_non_empty_string(value: str, parameter_name: str) -> None:
    """
    ...
    if not value.strip():
        raise ValueError(f"{parameter_name} cannot be empty or whitespace-only")


def _validate_identifier_list(identifiers: List[str], parameter_name: str = "identifiers") -> None:
    """
    Validate a list of player identifiers.
    ...
    Raises:
        ValueError: If the list or any identifier is invalid
    """
    if not identifiers or not isinstance(identifiers, list):
        raise ValueError(f"{parameter_name} must be a non-empty list")

    if len(identifiers) == 0:
        raise ValueError(f"{parameter_name} list cannot be empty")

    for i, identifier in enumerate(identifiers):
        if not identifier or not isinstance(identifier, str):
            raise ValueError(f"{parameter_name}[{i}] must be a non-empty string")

        if not identifier.strip():
            raise ValueError(f"{parameter_name}[{i}] cannot be empty or whitespace-only")

        if len(identifier.strip()) < 2:
            raise ValueError(f"{parameter_name}[{i}] must be at least 2 characters long")


def _validate_file_path(path: str, parameter_name: str) -> None:
    """
    Validate a file or directory path parameter.
    ...
    if '..' in path or path.startswith('/'):
        raise ValueError(f"{parameter_name} contains potentially unsafe path elements")
...
class NitradoAPIClient(DayZTool):
    ...
    def _setup_client(self) -> None:
        ...
        # Configurable SSL warning suppression
        suppress_ssl_warnings = self.get_config('nitrado_server.suppress_ssl_warnings', True)
        if suppress_ssl_warnings:
            urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
            logger.debug("SSL warnings suppressed via configuration")
    ...
    def make_request(self, endpoint: str, method: str = 'GET',
        ...
        try:
            ...
            error_msg = f"Nitrado API request failed for {method} {endpoint}"
            if hasattr(e, 'response') and e.response is not None:
                error_msg += f" (HTTP {e.response.status_code})"
                logger.error(f"{error_msg}: {e}")
                logger.error(f"Response content: {e.response.text[:500]}...")  # Limit response length
            else:
                logger.error(f"{error_msg}: {e}")
            raise
    ...
    def list_files(self, directory: str) -> List[Dict[str, Any]]:
        ...
        """
        # Validate input
        _validate_file_path(directory, "directory")
    ...
    def download_file(self, remote_path: str) -> bytes:
        ...
        """
        # Validate input
        _validate_file_path(remote_path, "remote_path")
    ...
    def add_to_list(self, list_type: str, identifiers: List[str]) -> Dict[str, Any]:
        ...
        Raises:
            ValueError: If list_type or identifiers are invalid
        """
        # Validate inputs
        _validate_non_empty_string(list_type, "list_type")
        _validate_identifier_list(identifiers, "identifiers")
        ...
        try:
            ...
            logger.error(f"Failed to add {len(identifiers)} players to {list_type} list: {e}")
            raise
    ...
    def remove_from_list(self, list_type: str, identifiers: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Failed to remove {len(identifiers)} players from {list_type} list: {e}")
            raise
    ...
    def is_player_banned(self, player_identifier: str) -> bool:
        ...
        Raises:
            ValueError: If player_identifier is invalid
            requests.RequestException: If the API request fails
        """
        # Validate input
        _validate_non_empty_string(player_identifier, "player_identifier")
