...
class LogFilterProfile(JSONTool):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # Note: JSONTool already calls initialize_directories() in its __init__
        ...
        if config_dir:
            ...
            # Use output_dir as base since it's already initialized
            self.config_dir = Path(self.output_dir) / "log_profiles"
    ...
    def save_profile(self, name: str, start_date: str = None, end_date: str = None,
        ...
        try:
            ...
            logger.info(f"Saved log filter profile '{name}' to {profile_path}")
            return True
        except (IOError, OSError) as e:
            logger.error(f"Error saving log filter profile '{name}': {e}")
            return False
...
def main():
    ...
    # Create common filters command
    subparsers.add_parser('create-common', help='Create a set of common filter profiles')
