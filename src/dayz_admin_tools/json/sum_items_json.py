...
from ..base import JSONTool, XMLTool, logger
...
class SumItemsJson(XMLJSONTool):
    """
    ...
    def run(self, json_files: List[str], output_csv: Optional[str] = None,
            types_xml: Optional[str] = None) -> Dict[str, Dict[str, Any]]:
        """
    ...
    def process_json_files(self, json_files: List[str], output_csv: Optional[str] = None,
                           types_xml: Optional[str] = None) -> Dict[str, Dict[str, Any]]:
        """
        Process multiple JSON files and count item occurrences.
        ...
        if has_types_file:
            logger.info(f"Validating items against types.xml with {len(valid_items)} items")
        else:
            logger.warning("No types.xml file provided or none found, items won't be validated")
        ...
        for filename in json_files:
            ...
            try:
                ...
                if "Objects" in data:
                    ...
                    logger.warning(f"No 'Objects' key found in {filename}")
        ...
        if invalid_data_rows and has_types_file:
            ...
            logger.info(f"Invalid items written to {invalid_output_path}")

        # Count valid and total items for logging
        valid_item_count = len(valid_data_rows)
        total_item_count = len(item_data)

        logger.info(f"Processed {len(json_files)} files, found {total_item_count} unique items")

        # Log statistics about item filtering
        if has_types_file:
            invalid_item_count = total_item_count - valid_item_count
            logger.info(f"Item validation: {valid_item_count} valid items included, "
                        f"{invalid_item_count} invalid items excluded")
    ...
    def parse_types_xml(self, types_xml_path: Optional[str] = None) -> Set[str]:
        ...
        if not types_xml_path:
            types_xml_path = self.get_config('paths.types_file')

        if not types_xml_path:
            logger.warning("No types.xml file provided and none found in configuration")
            return valid_items

        try:
            # Use the read_xml method from XMLTool base class
            root = self.read_xml(types_xml_path)
            logger.info(f"Parsing types.xml from {types_xml_path}")

            # Extract all type names using direct attribute access
            for type_elem in root.findall('type'):
                name = type_elem.get('name')
                if name:
                    valid_items.add(name)

            logger.info(f"Found {len(valid_items)} valid items in types.xml")
            ...
            if "XML" in str(type(e).__name__) or "parse" in str(e).lower():
                ...
                logger.error(f"Error parsing types.xml: {e}")
                raise
...
def main():
    ...
    parser.add_argument("--output", "-o", dest="output_csv",
                        help="Path to the output CSV file. If not provided, "
                             "a default name will be generated in the standard output directory.")
    parser.add_argument("--types-xml", "-t", dest="types_xml",
                        help="Path to types.xml file for item validation. "
    ...
    if invalid_items > 0:
        print("Invalid items written to separate CSV file with '_invalid' suffix")
