...
Uses 'paths.types_file', 'paths.mapgroupproto_file', and 'paths.cfglimitsdefinition_file' from the profile configuration by default.
By default, it checks both types.xml and mapgroupproto.xml in a single run.
"""
...
class CheckUsageTagsTool(XMLTool):
    ...
    def check_invalid_usages_proto(self, xml_file: str, valid_usages: Set[str]) -> Dict[str, Dict[str, int]]:
        ...
        try:
            ...
            if not invalid_usages_data:
                ...
                logger.warning(f"Found {len(invalid_usages_data)} invalid usage tags across {total_groups} groups with {total_occurrences} total occurrences")
    ...
    def check_invalid_usages_types(self, xml_file: str, valid_usages: Set[str]) -> Dict[str, int]:
        ...
        try:
            ...
            if not undefined_usages:
                ...
                logger.warning(f"Found {len(undefined_usages)} invalid usage tags with {total_occurrences} total occurrences")
                # Log the top 5 most common invalid usages for quick reference
    ...
    def run(self, cfglimits_file: Optional[str] = None, xml_file: Optional[str] = None, check_both: bool = True, csv_file: Optional[str] = None, summary_csv_file: Optional[str] = None) -> Dict[str, Any]:
        """
        Run the CheckUsageTags tool.
        
        Args:
            cfglimits_file: Path to the cfglimitsdefinition.xml file (uses paths.cfglimitsdefinition_file from config if None)
            xml_file: Path to the XML file to check (uses paths.types_file or paths.mapgroupproto_file from config if None)
            ...
            summary_csv_file: Optional path for the summary CSV output file (uses general.output_path from config if None)
        ...
        if not cfglimits_file:
            ...
            if not cfglimits_file:
                logger.error("No cfglimits file specified and no 'paths.cfglimitsdefinition_file' configured in profile")
                return {"error": "No cfglimits file specified and no 'paths.cfglimitsdefinition_file' configured in profile"}
        ...
        try:
            ...
            if 'mapgroupproto' in xml_path.lower():
                ...
                logger.error("Unsupported XML file type. Please provide either a mapgroupproto.xml or types.xml file.")
                return {"error": "Unsupported XML file type. Please provide either a mapgroupproto.xml or types.xml file."}
    ...
    def check_both_files(self, cfglimits_path: str, types_file: str, mapgroupproto_file: str, csv_file: Optional[str] = None, summary_csv_file: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            summary_csv_file: Optional path for the summary CSV output file (uses general.output_path from config if not specified)
...
def main():
    ...
    parser.add_argument("--cfglimits", 
        help="Path to the cfglimitsdefinition.xml file (uses paths.cfglimitsdefinition_file from profile config if not specified)")
    parser.add_argument("--xml_file", 
        help="Path to a specific XML file to check (if not specified, both types.xml and mapgroupproto.xml from config will be checked)")
    parser.add_argument("--mapgroupproto_only", action="store_true",
        help="Check only mapgroupproto.xml instead of both files (uses paths.mapgroupproto_file from profile config)")
    parser.add_argument("--types_only", action="store_true",
        help="Check only types.xml instead of both files (uses paths.types_file from profile config)")
    parser.add_argument("--csv", 
        help="Custom path for CSV output file (uses general.output_path from profile config if not specified)")
    parser.add_argument("--summary-csv", 
        help="Custom path for summary CSV output file with invalid usage tags and their counts (uses general.output_path from profile config if not specified)")
    ...
    result = tool.run(args.cfglimits, xml_file, check_both=check_both, csv_file=args.csv, summary_csv_file=args.summary_csv)
