    ...
    def run(self, file1: str, file2: str, output_csv: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if not output_csv:
                ...
                base_name = self.generate_timestamped_filename(
                    f"types_comparison_{Path(file1).stem}_{Path(file2).stem}", "csv")
...
def main():
    ...
    parser.add_argument(
        "--output-dir", help="Directory to store output files (overrides general.output_path from config)")
    ...
    if args.output_dir:
        ...
        base_name = tool.generate_timestamped_filename(
            f"types_comparison_{Path(args.types_xml1).stem}_{Path(args.types_xml2).stem}", "csv")
