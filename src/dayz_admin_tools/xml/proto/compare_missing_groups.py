...
class MissingGroupsComparer(XMLTool):
    ...
    def compare_missing_groups(self) -> bool:
        ...
        try:
            ...
            if resolved_path:
                ...
                logger.info(f"Found {len(missing_in_file1)} groups missing in {file1_name} and {len(missing_in_file2)} groups missing in {file2_name}")
    ...
    def setup_from_args(self, args: argparse.Namespace) -> None:
        ...
        self.output_file = self._get_output_path(output_filename) or os.path.join(self.output_dir, "missing_groups_comparison.csv")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=
            'Compare mapgroupproto.xml files to identify missing group names.\n'
            'Features:\n'
