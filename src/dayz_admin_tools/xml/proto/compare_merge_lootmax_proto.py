...
class LootmaxComparer(XMLTool):
    ...
    def compare_lootmax(self) -> bool:
        ...
        try:
            ...
            if resolved_path:
                ...
                logger.info(f"CSV format uses separate 'Group Name' and 'Container Name' columns to represent hierarchy")
    ...
    def merge_lootmax(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Successfully merged lootmax values from {os.path.basename(self.file2)} into {os.path.basename(self.file1)}")
            return True
    ...
    def setup_from_args(self, args: argparse.Namespace) -> None:
        ...
        self.output_file = self._get_output_path(output_filename) or os.path.join(self.output_dir, "lootmax_comparison.csv")
...
def main():
    ...
    parser.add_argument('--merge', '-m',
                      ...
                      help='Enable merging and optionally provide a filename (defaults to merged_mapgroupproto.xml with timestamp)')
